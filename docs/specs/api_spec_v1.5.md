# kabuhandan_hojo API Spec v1.5

## 1. scope

この版は v1.4 の API 契約を累積継承し、次の2つを追加する。

- 登録済み個別銘柄1件を対象とする最小AI縦スライス `POST /api/ai/analyses`
- その縦スライスだけを操作する独立HTML shell `GET /ui/analysis`

v1.4 の multi-mode stock AI review、Prompt Registry / Prompt Builder、mode別Structured Outputs、Web検索制御、JSON parse救済、ChatGPT手動投入用プロンプト生成は引き続き有効である。ただし、それらは legacy stock-review 経路の契約であり、新しい個別銘柄AI縦スライスには継承しない。

### 1.1 AI経路の境界

| 項目 | 個別銘柄AI縦スライス | legacy stock-review |
|---|---|---|
| canonical path | `POST /api/ai/analyses` | `POST /api/ai/stock-review` |
...
この経路ではbackground、streaming、pollingを行わない。
...
|---|---|---|
| common OS | `common_os@2026.08.16` | `1. 株判断共通OS` |
| common input | `common_input_rules@2026.08.16-mvp1` | `2. 共通入力テンプレート`のMVP必要部分 |
| execution constraints | `execution_constraints_no_tools@mvp1` | Web・外部市場データなしのアプリ制約 |
| task module | `individual_comprehensive@2026.08.16` | `3.1 総合的な個別銘柄分析` |

prompt versionは `2026.08.16`、profileは `individual_security_comprehensive`、compiler versionは `individual-security-v1`、module IDは `3.1` である。
...
`AiAnalysisResponse`へprompt traceを公開しない。現行実装はtrace専用DB/audit storeを持たず、後日の相関可能期間は通常ログとOpenAI側metadataの保持期間に依存する。恒久保存を保証する契約ではない。
...
したがってこの版の新endpointはtrusted local development environment向けで、Internetや信頼できないLANへ直接公開してはならない。外部公開前に認証、rate limit/cost control、TLS、host設定、監査保存を別フェーズで実装する必要がある。これらを実装済みとはみなさない。

## 18. OpenAPI declaration gap

現行FastAPI routeの自動生成OpenAPIは、`POST /api/ai/analyses`について200と422だけを宣言する。実装が返す404、429、502、503、504および内部500は、第15節の実行時契約には存在するが、route decoratorの`responses`としてOpenAPIへまだ宣言されていない。
...
- live browser checkは銘柄検索、選択、質問、送信、loading、実endpoint response、answer表示、error非表示を確認する。
...
- prompt traceは通常responseへ公開せず、永続保存しない。
