# kabuhandan_hojo API Spec v1.9

## 1. scope

この版は v1.8 の API 契約を累積継承し、次を追加・更新する。
...
数字4桁に限定せず、英字を含む公開コード（例:`285A`）と、登録済みraw identifier（例:`285A0`）を検索できる。返却する`SecuritySearchResult.ticker_code`は、該当する`security_master`のraw identifierである。dashboardは英字を含む5文字末尾`0`形式だけを公開4文字へ変換して表示・portfolio入力し、detail actionのdata属性とURLにはraw返却値を維持する。検索は保有recordを作成せず、検索結果の選択だけで`POST /portfolio`を呼ばない。
