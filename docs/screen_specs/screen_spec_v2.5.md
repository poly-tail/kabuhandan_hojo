# kabuhandan_hojo Screen Spec v2.5

## 1. scope

この版は、v2.4の画面契約を累積継承し、dashboard legacy AIの構造化回答を、意味のある見出し、箇条書き、callout、根拠badgeを使って読みやすく表示します。legacy成功responseはMarkdown本文ではなくStructured Outputsで得たJSON objectであり、画面は既知fieldを安全な表示componentへ対応付けます。model由来の文字列をHTMLまたは汎用Markdownとして実行しません。Portfolio / Watchlistは同じ表示helperと安全規則を使います。API schema、OpenAI request、model、status、quota、cache/history、canonical個別銘柄AI画面、active prompt v2026.08.18、銘柄master同期、回答readerの契約はv2.4から変更しません。
