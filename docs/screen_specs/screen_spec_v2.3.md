# kabuhandan_hojo Screen Spec v2.3

## 1. scope

この版は、v2.2の画面契約を累積継承し、dashboard legacy AIの軽量スキャンで構造化できなかったOpenAI応答を成功表示しないようにします。`json_parse_failed`を赤いerror cardで表示し、JSON構文、JSON root、JSON項目形式の失敗を利用者向けlabelで区別します。生応答は調査用に表示・履歴保存できる一方、成功回数とcacheには含めません。canonical個別銘柄AI画面、銘柄master同期、回答readerの契約はv2.2から変更しません。
