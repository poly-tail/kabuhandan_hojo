# kabuhandan_hojo Screen Spec v2.4

## 1. scope

この版は、v2.3の画面契約を累積継承し、dashboard legacy AIの銘柄候補一覧がコードだけになる表示不整合を修正します。銘柄別cardは正式名称と公開codeを併記し、portfolio summaryの銘柄候補listは「銘柄名（公開コード）」で表示します。英字末尾`0` raw codeのキオクシアは`キオクシアホールディングス（285A）`、名称を解決できないcodeは`名称未登録（code）`と表示します。live、mock、既存cache hitで同じ表示規則を使います。canonical個別銘柄AI画面、active prompt v2026.08.18、銘柄master同期、回答readerの契約はv2.3から変更しません。
