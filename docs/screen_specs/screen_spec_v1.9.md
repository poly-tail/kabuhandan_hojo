# kabuhandan_hojo Screen Spec v1.9

## 1. scope

この版は、v1.8の画面契約を累積継承し、canonical個別銘柄AIの回答生成成功とローカル保存結果を分離する表示を追加します。保存失敗でも回答本文を維持してwarningを表示し、保存成功時だけ保存済み表示と別ウィンドウreader導線を出します。
