# kabuhandan_hojo Screen Spec v1.7

## 1. scope

この版は、v1.6 の dashboard / detail / chart と既存 dashboard Portfolio AI分析パネルの契約を累積継承し、独立した個別銘柄AI分析画面 `/ui/analysis` を追加した現行画面仕様を対象にします。
...
- UI: `/ui/analysis`
- canonical API: `POST /api/ai/analyses`
...
- Web検索、mock、prompt-only、prompt編集、結果保存、履歴、複数銘柄選択のcontrolは置かない
