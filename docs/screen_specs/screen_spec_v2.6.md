# kabuhandan_hojo Screen Spec v2.6

## 1. scope

この版は、v2.5の画面契約を累積継承し、dashboard legacy AIで現在表示している構造化回答または解析失敗時の生応答を、client-onlyのBlob snapshotとして別タブまたは別ウィンドウへ大きく表示できるようにします。Portfolio / Watchlistは同じ表示helper、安全規則、reader生成処理を使います。新しいAPI route、DB保存、Web Storage、OpenAI再呼び出しは追加しません。canonical個別銘柄AIの保存済み回答reader、API schema、OpenAI request、model、status、quota、cache/history、active prompt v2026.08.18、銘柄master同期の契約はv2.5から変更しません。
