# kabuhandan_hojo Screen Spec v2.7

## 1. scope

この版はv2.6の画面契約を累積継承し、dashboardで別々だったPortfolioとWatchlistを、selectorで切り替える1つの全幅「保有・ウォッチリスト管理」spaceへ統合します。Portfolioは独立storageのまま既定表示とし、default「メイン」を含む複数named watchlistを作成・名前変更・選択・削除できます。検索、Focus Board、alerts、銘柄選択、詳細 / chart遷移、legacy AI targetは選択中`watchlist_id`へscopeします。memo / thesisはsecurity-levelで共有し、同じ銘柄を複数listへ登録できます。明示named listが空でもmock銘柄を表示・分析しません。認証・利用者分離は追加せず、全collectionはアプリ全体で共有されます。
