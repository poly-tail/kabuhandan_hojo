# kabuhandan_hojo Screen Spec v2.2

## 1. scope

この版は、v2.1の画面契約を累積継承し、dashboardの銘柄検索領域から利用者自身のJ-Quants APIキーで東証/J-Quants上場銘柄マスターをprivateなローカルDBへ同期し、完全性、情報基準日、同期時刻、件数を確認できる導線を追加します。完全な一覧は画面やpublic repositoryへ埋め込まず、地方取引所単独銘柄まで含む「全日本銘柄」とは表示しません。
