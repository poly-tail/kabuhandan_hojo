# kabuhandan_hojo Screen Spec v2.1

## 1. scope

この版は、v2.0の画面契約を累積継承し、dashboardの銘柄検索結果からportfolio入力フォームへ選択銘柄を安全に引き継ぐ導線を追加します。銘柄名、数字コード、英字を含むコードを検索でき、各結果で「保有入力へ」と「詳細を見る」を明示的に分離します。「保有入力へ」は銘柄identifierのprefillと数量欄focusだけを行い、利用者が数量を入力して保存するまでは保有recordを作りません。
