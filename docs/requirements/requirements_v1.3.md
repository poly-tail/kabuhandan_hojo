...
v1.3では、v1.2のcanonical個別銘柄AI縦スライスとlegacy portfolio向けmulti-mode AI経路を維持しつつ、成功回答のローカルSQL永続保存、request IDによる1件取得、大型の別ウィンドウ回答画面を追加する。また、個別銘柄prompt sourceをv2026.08.17へ更新し、銘柄の表示・言及を原則として「銘柄名（銘柄コード）」とする。
...
canonical個別銘柄経路では、v1.3時点で新たなWeb検索、J-Quants同期、外部市場データ取得を実行しない。利用可能なcontextは、登録済み`security_master`の以下の項目に限定する。
...
- `preset`: v1.3では`STANDARD`のみ。未指定時も`STANDARD`とする。
...
- 保存に失敗した場合はtransactionをrollbackし、成功responseを返さず、`PERSISTENCE_ERROR`のHTTP 500として扱う。
- OpenAI API側の保存期間をアプリ履歴の代替とせず、ローカルSQL recordを再表示の正本とする。
...
v1.3で使用する元資料は「株判断プロジェクト｜定型プロンプト集 v2026.08.17」とし、次のassetだけを組み込む。
...
v2026.08.17の命名規則を反映し、銘柄の表示・言及は原則として「銘柄名（銘柄コード）」とする。外国銘柄を将来扱う場合は「会社名（ticker）」とする。共通入力では銘柄名と銘柄コードを別項目として扱うが、現行MVPのrequest fieldは互換性のため`security_code`を維持し、銘柄名は解決済み`security_master`から渡す。
...
- 成功回答が`request_id`でSQLへ保存され、保存失敗時はrollbackして`PERSISTENCE_ERROR`になる。
...
評価dimensionは最低限、観点の広さ、反証条件、情報不足の扱い、冗長性とする。v1.3では投資判断の正しさを自動採点しない。
...
- canonical成功回答はローカルSQLへ保存するが、APIキー、prompt全文、provider raw responseは保存しない。
...
20. 保存失敗ではrollbackし、HTTP 500の`PERSISTENCE_ERROR`を返してsuccessにしない。
21. `GET /api/ai/analyses/{request_id}`で保存済み回答を1件取得でき、存在しないIDをsuccess扱いしない。
22. `/ui/analysis`の成功時だけ大型表示導線が現れ、新規windowで`/ui/analysis/results/{request_id}`を開いて回答本文を表示できる。
23. 保存recordと公開responseへAPIキー、prompt全文、provider raw responseを含めない。
24. prompt sourceがv2026.08.17で、共通OSの命名規則、必要な共通入力rule、no-tools制約、module 3.1だけが実requestへ入る。

## 16. v1.3の非対象

次はv1.3のcanonical個別銘柄経路へ実装しない。
...
- canonical endpointにはアプリ独自の認証、server-side rate limit、daily quota、TLS終端がなく、runnerの既定bind hostは`0.0.0.0`である。現状はtrusted local環境限定とし、Internetへ直接公開しない。
...
- canonical経路とlegacy経路ではWeb、Structured Outputs、mock、cache、fallbackの方針が異なる。この差異はv1.3時点の意図的な責務分離である。
