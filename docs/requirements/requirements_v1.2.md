...
v1.2では、v1.1までのportfolio向けmulti-mode AI経路を維持しつつ、登録済み個別銘柄1件を対象とするcanonicalなAI最小縦スライスを別経路として定義する。また、この新経路へ株判断用定型プロンプトの必要部分だけをversioned assetとして組み込む。
...
canonical個別銘柄経路では、v1.2時点で新たなWeb検索、J-Quants同期、外部市場データ取得を実行しない。利用可能なcontextは、登録済み`security_master`の以下の項目に限定する。
...
|---|---|---|
| 主endpoint | `POST /api/ai/analyses` | `POST /api/ai/stock-review`および既存互換endpoint |
| 主UI | `GET /ui/analysis` | dashboardのPortfolio AI分析パネル |
...
- `preset`: v1.2では`STANDARD`のみ。未指定時も`STANDARD`とする。
...
v1.2で使用する元資料は「株判断プロジェクト｜定型プロンプト集 v2026.08.16」とし、次のassetだけを組み込む。
...
回答本文は`textContent`と`white-space: pre-wrap`によるプレーンテキスト表示とする。Markdown renderer、HTML挿入、構造化cardを導入しない。browserからOpenAI APIを直接呼ばず、APIキーやprompt本文をbrowserへ送らない。
...
評価dimensionは最低限、観点の広さ、反証条件、情報不足の扱い、冗長性とする。v1.2では投資判断の正しさを自動採点しない。
...
- canonical経路には専用のローカル分析履歴tableがない。prompt provenanceはOpenAI metadata、OpenAI response ID、安全なserver log識別子による追跡に依存する。
...
- canonical経路とlegacy経路ではWeb、Structured Outputs、mock、cache、fallbackの方針が異なる。この差異はv1.2時点の意図的な責務分離である。
