...
v1.7では、v1.6の機能を累積継承し、利用者自身のJ-Quants APIキーで東証/J-Quants上場銘柄マスターをローカルDBへ同期し、銘柄検索の母集団を拡張できるようにする。対象はJ-Quantsの上場銘柄マスターが返す東証上場銘柄・ETF・REIT・優先株等であり、名古屋・福岡・札幌等の地方取引所単独銘柄を含む「日本全国の全取引所銘柄」は保証しない。完全な銘柄一覧は公開repositoryへ同梱・再配布せず、36件のseedは初期検索用の限定fallbackとしてだけ保持する。
