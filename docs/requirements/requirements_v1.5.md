...
v1.5では、v1.4の機能を累積継承し、dashboardのlegacy stock-review経路に限って、アプリ内日次quotaの既定値を300へ変更する。成功した一括reviewとOpenAI Responses APIのprovider callを分離してJST日次・月次で集計し、token使用量に基づく概算額と未算定callを表示する。canonical個別銘柄AI経路`POST /api/ai/analyses`のmodel、preset、保存、quota契約は変更しない。
