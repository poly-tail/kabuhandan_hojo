...
v2.0では、v1.9の機能を累積継承し、Portfolioと複数のnamed watchlistをdashboardの1つの管理spaceで切り替えられるようにする。Portfolioの保存先は`portfolio_holding`のまま分離し、監視銘柄はsecurity-levelの`watchlist`正本と、named collection / membershipを表す`watchlist_collection` / `watchlist_membership`へ分ける。同じ銘柄を複数listへ登録できる一方、memoと強気・弱気仮説は銘柄単位で共有する。既存`/watchlist`はdefault collection「メイン」の互換入口として維持し、既存DBは起動時に一度だけdefaultへbackfillする。named list選択時は検索、dashboard、詳細遷移、AI targetを同じ`watchlist_id`へscopeし、空listをmock銘柄へfallbackしない。認証・利用者分離は追加せず、collectionはアプリ全体で共有される。
