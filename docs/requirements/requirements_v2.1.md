...
v2.1では、v2.0の機能を累積継承し、dashboard legacy stock-reviewでローカル保存した回答を最新100件まで一覧・詳細表示し、Markdownとして保存できるようにする。履歴一覧は分析方法で分類し、mode / target / statusで絞り込める。詳細表示、Markdown保存、別タブ印刷表示は保存済みrecordだけを読み、OpenAIを再呼び出さない。browserの印刷機能からPDFへ保存できるが、serverはPDF fileを生成しない。legacy保存recordに当時のnamed watchlist名は含まれないため、古い履歴ではlist名を復元せず、保存済み`watchlist_id`または一般的な対象labelだけを表示する。canonical個別銘柄AIの保存・取得契約は変更しない。
...
### 4.3 Portfolio・複数named watchlist管理

#### 保存modelと共有範囲

- Portfolioは`portfolio_holding`を正本とし、watchlist collectionやmembershipへ変換しない。保有数量、平均取得単価、保有メモ等はnamed watchlistとは別に保存する。
- 既存`watchlist`は1銘柄につき1つのsecurity-level監視recordを正本とし、`memo`、`thesis_bull`、`thesis_bear`を保持する。同じtickerを複数named listへ登録しても`watchlist` rowを複製しない。
- `watchlist_collection`は利用者向けname、NFKC・trim・casefold済みidentity、system key、表示順、有効状態、timestampを持つ。system defaultは`system_key=default`の「メイン」1件とする。
- `watchlist_membership`はcollectionとsecurity-level `watchlist` rowの関連、list内表示順、有効状態、timestampを持つ。同じcollection / itemの組合せを重複作成しない。
- 同じ銘柄は複数collectionへ同時に所属できる。membershipの追加・解除とlist削除は、Portfolioおよび他collectionのmembershipへ影響させない。
- memoと強気・弱気仮説はcollection固有fieldにせずsecurity-levelで共有する。あるlistまたは詳細画面から変更すると、その銘柄を含む他listにも同じ値が反映される。
- collection名はNFKC正規化して前後空白を除いた1〜80文字とし、casefoldしたidentityで一意にする。たとえば`ＴＥＣＨ`と`tech`を別listとして作成しない。
- default collectionは削除できない。nameと表示順の更新は許可する。non-default collectionは明示確認後に削除でき、そのcollectionとmembershipだけを削除する。

#### 既存DBと互換入口

- 起動時migrationは`watchlist_collection`と`watchlist_membership`をSQLite / PostgreSQLの双方で準備する。
- `system_key=default`が存在しない初回だけ、default collection「メイン」の作成または同名collectionへのsystem key付与と、既存legacy `watchlist` rowのmembership backfillを同じtransactionで行う。
- legacy DBに同tickerの複数rowがある場合は、active、更新時刻、IDの決定順で代表rowを選び、legacy row自体を破壊的に統合しない。
- default system keyが既に存在する再起動では、全`watchlist` rowをdefaultへ再backfillしない。named listにだけ追加した銘柄を後からdefaultへ漏らさない。
- legacy `GET /watchlist`と`POST /watchlist`は、`watchlist_id`未指定時のdefault collection互換入口として維持する。
- named collectionのCRUD、membership一覧・追加・解除は新しい`/watchlists`経路で行う。存在しないまたはinactiveなcollection/itemはnot found、正規化後の重複nameとdefault削除はconflictとする。

#### dashboardとAI scope

- `/ui/dashboard`はPortfolioとwatchlistを別panelへ常時並べず、1つの全幅「保有・ウォッチリスト管理」spaceへ統合する。
- native selectorは`portfolio`と`watchlist:<collection_id>`を選べ、query指定がない初期表示はPortfolioとする。named listでは作成・名前変更・non-default削除、検索結果からの追加、cardからのmembership解除を同じspaceで行う。
- dashboard view modelはactive collection一覧、選択中collection ID、選択collectionのitemsを返す。`watchlist_id`を指定しない場合、またはactive collectionとして解決できない場合はdefault collectionを選ぶ。browserはrequest開始時のscopeを保持し、responseのrequest IDとrequested `watchlist_id`が現在activeなscopeと一致する場合だけ描画する。
- named list選択中は検索の`in_watchlist`、watchlist item、Focus Board、alert等のwatchlist由来表示を同じcollectionへscopeする。Portfolioの保存・一覧は同じselectorで切り替えても変更しない。
- named listの銘柄選択checkboxはcollection IDごとにbrowser memoryで分離し、再読込後に存在しないtickerを除く。別collectionの選択を混ぜない。1件以上を選択した場合は共通AI targetを`selected`へ自動変更し、全解除時にmanual tickerがなければ`watchlist`へ戻す。
- named listからdetail / chartを開く場合はURL queryの`watchlist_id`を維持し、dashboardへの戻りlink、detail画面のwatchlist追加、仮説保存を同じcollectionへscopeする。Portfolio表示から開いた場合はqueryを付けず、detailのwatchlist操作はlegacy default endpointを使う。
- URL `?watchlist_id=`からdashboardを開いた場合はそのnamed listを初期表示し、共通AI targetも`watchlist`へ初期化する。Portfolioへ戻る場合はPortfolio表示だけでなくdefault watchlistのmonitoring scopeも再取得し、named list由来のFocus Board / alertを残さない。
- legacy AI controlsと現在回答のreaderは統合spaceで共通利用する。named listへ切り替えた場合、AI requestは`target=watchlist`とその`watchlist_id`を送る。実行開始時のlist名をclient-side context snapshotとして保持し、処理中または完了後にrename / switchされてもsummary labelと別タブtitleを別list名へ書き換えない。
- selector切替、collection作成、active collection削除、active membershipの追加・解除、checkbox選択変更では、現在のlegacy AI結果とreader actionを無効化する。進行中requestもrequest IDで無効化し、旧scopeのresponseを再表示しない。名前変更だけは実行済みsnapshotを書き換えない。
- 明示されたnamed listが空、存在しない、またはinactiveの場合はAI対象を空のwatchlistとして扱い、mock holdingsへfallbackせず`no_holdings`を返す。`watchlist_id`を持たないlegacy targetの既存default / mock互換は維持する。
- collection、membership、dashboard、AI targetに利用者IDを追加しない。現行のnamed listは認証・利用者分離のないアプリ全体の共有dataであり、trusted local環境だけで利用する。
...
- 保有銘柄の一括分析
- defaultまたは明示したnamed watchlistの一括分析
- 狙い中銘柄の一括分析
- 個別または少数銘柄の詳細分析
- portfolio全体の買い売り判断
- 重要局面分析
...
対象は保有銘柄、狙い中銘柄、監視銘柄、選択銘柄、テスト用仮銘柄から選べる。Portfolio / named watchlistのselectorとAI controlsは同じ管理spaceに置き、named listへ切り替えた場合はtargetを`watchlist`へ揃え、そのcollection IDを`watchlist_id`として送る。明示named listが空の場合は対象0件をそのまま表示し、テスト用仮銘柄へ置き換えない。prompt入力欄が空でもPrompt Builder templateを適用し、高cost modeやWeb検索ONでは実行前に確認できる。warnings、sources、銘柄別card、portfolio総合判断、執行案、反証条件、辛口check、履歴・前回結果を表示する。`status=json_parse_failed`は成功色を使わず赤いerror cardとし、`schema_validation`は「JSON項目形式エラー」、`root_shape`は「JSONルート形式エラー」、`json_syntax`は「JSON構文エラー」と区別する。表示可能な`raw_model_output`は同じerror card内で確認できる。利用量panelには本日の`review_runs / daily_limit`、残数、本日概算、今月の`review_runs`と概算、未算定API call、v2開始前履歴が不完全である旨を表示する。
...
- `target=watchlist`かつ非nullの`watchlist_id`を持つ明示named listは、空または未検出でもmock fallbackを行わない。`watchlist_id`を持たない既存callerのdefault watchlist / mock互換だけを維持する。
...
### 13.8 legacy保存履歴の一覧・詳細・Markdown export

- legacy stock-reviewはrequestの`save_result=true`で新しい結果を生成した場合だけ、その結果をgit管理外の`data/ai_review_history.json`へ保存する。`save_result=false`は保存せず、cache hitは`save_result=true`でも新しい履歴recordを作らない。
- 保存対象は`status=success`のlive / mock / forced mock、`prompt_only`、および調査可能な`json_parse_failed`である。対象銘柄なし、API key不足、quota拒否、OpenAI error等、結果responseを生成する前に終了した失敗は保存しない。
- prompt / mock / live / raw fallbackの履歴保存に失敗しても生成済みresponseを返し、OpenAIを再呼び出さず、固定の安全なwarningを`warnings`へ1回だけ追加する。保存済みと誤表示せず、そのresponseをcacheへ保存しない。
- history rootは最大100 recordを保持し、新しいrecordを追加するときは古いrecordから削る。履歴IDは保存内容から決定的に作る64文字の小文字SHA-256 hexとし、秘密値や認可tokenとして扱わない。
- 履歴一覧は保存順の新しいものから返し、`mode`、`target`、`status`、`limit`、`offset`で絞り込み・paginationできる。`limit`は1〜100、既定100である。一覧は表示用metadataだけを返し、回答本文、`request_payload`、prompt全文、provider raw responseを返さない。`summary`もmodel本文ではなくmode / status / 銘柄数から決定的に作る。
- 一覧の`total`はfilter後かつpagination前の件数、`mode_counts`はfilter前の有効record全体のmode別件数、`stored_count`は保存rootの総entry数、`invalid_count`は読み飛ばした不正entry数とする。
- 詳細取得は`{history_id, review}` envelopeを返し、保存recordに残る`request_payload` keyを公開responseから除外する。過去のcode-only recordはread時だけ公開code・正式名称へ補完し、履歴fileを書き換えない。
- Markdown exportはsemanticなUTF-8 Markdown attachmentとし、安全なASCII filename、`Cache-Control: no-store`、`X-Content-Type-Options: nosniff`、`Referrer-Policy: no-referrer`を付ける。可変textをMarkdown/HTMLとしてescapeし、sourceはHTTP(S)だけをlinkにし、raw outputのcode fence長を内容に合わせる。
- dashboardは履歴を分析方法ごとに分類し、一覧から安全な詳細表示、`.md`保存、別タブ印刷表示へ移動できる。別タブ表示は既存の共通safe rendererと制限付きCSPを使い、browserの印刷操作でPDF保存できる。PDFの直接生成、server保存、browser Web Storage保存は行わない。
- raw fallbackの画面表示は先頭20,000文字までに制限でき、省略時はその旨と全文をMarkdown保存で確認できることを表示する。印刷用cloneではraw outputの`details`をopenにして表示範囲をPDFから欠落させない。
- 一覧・詳細・Markdown export・印刷表示はローカル保存済みrecordのreadだけで、OpenAI API、quota counter、usage ledger、cacheを更新しない。
- 履歴JSONの通常更新はprocess内`RLock`、一時file、flush / fsync、`os.replace`によるatomic replaceで行う。不正entryは一覧から読み飛ばして件数を示すが、root自体が不正な場合は既存fileを上書きしない。
- この履歴APIと画面はlegacy stock-review専用であり、canonical `AiAnalysisRecord`の一覧・exportを追加しない。
...
- 通信失敗とUI表示失敗を混同しない。
- named watchlist collectionとmembershipは認証・owner IDを持たないアプリ全体の共有dataとして扱い、既定のloopbackとtrusted local運用を維持する。
- collection名、銘柄名、memo、仮説等の可変textはUIでescapeし、IDはpositive integerとしてvalidationする。
...
72. 起動時にdefault collection「メイン」とmembership tableが準備され、defaultが存在しない初回だけ既存legacy watchlist rowを同じtransactionでbackfillする。
73. migration再実行ではnamed listにだけ存在する銘柄をdefault collectionへ漏らさず、SQLite / PostgreSQLの双方で起動できる。
74. Portfolioが`portfolio_holding`のままwatchlist storageと分離され、selector切替、collection削除、membership解除で保有recordが変わらない。
75. 同じtickerを複数named listへ登録してもsecurity-level `watchlist` rowは1件で、collectionごとのmembershipと表示順だけが分離される。
76. memo、強気仮説、弱気仮説がsecurity-levelで共有され、あるlistからの更新を他listでも同じ値として参照できる。
77. collection名をNFKC・trim・casefoldで同一判定し、空名・80文字超を422、重複名を409として拒否する。
78. default collectionを一覧の先頭に返し、name / sort orderは更新できる一方、削除requestは409となる。non-default削除は204で、他listとPortfolioへ影響しない。
79. `GET/POST /watchlist`がdefault collectionの既存互換入口として動き、新しい`/watchlists` CRUDとcollection item APIがnamed listを扱う。
80. `GET /securities/search?watchlist_id=...`の`in_watchlist`が指定collectionだけを表し、同じ銘柄を別listへ追加してもsecurity masterを複製しない。
81. `GET /ui/dashboard/data?watchlist_id=...`がcollection一覧、解決済み`selected_watchlist_id`、そのlistのitem / focus / alertを返し、未指定または解決不能IDはdefaultへfallbackする。
82. dashboardがPortfolioとnamed watchlistを1つの全幅管理spaceで切り替え、初期表示はPortfolio、selector値は`portfolio` / `watchlist:<id>`となる。
83. UIからnamed listをinline作成・名前変更・non-default削除でき、default削除buttonはdisabledとなる。
84. named list選択中の検索結果からそのlistへ銘柄を追加し、cardからそのmembershipだけを解除できる。空listではmock itemを表示しない。
85. checkbox選択をcollection IDごとに分離し、reload後に現在listに存在しないtickerを除いて、別listの選択をAI対象へ混ぜない。
86. named listからdetail / chartを開くURL、戻るlink、詳細画面の追加・仮説保存が同じ`watchlist_id`を維持し、Portfolio起点ではlegacy default watchlist endpointを使う。
87. named list全体のlegacy AI requestが`target=watchlist`と正しい`watchlist_id`を送り、明示named listが空または未検出でもmock fallbackせず`no_holdings`となる。
88. mock modeでもnamed collection CRUD、membership scope、dashboard / search scope、named AI targetの同じ外部契約を検証できる。
89. collectionとmembershipに利用者IDや認証を持たないこと、全listがアプリ全体で共有されること、Internetへ直接公開しないことがREADMEと仕様で確認できる。
90. URL `?watchlist_id=`で初期表示したnamed listでは共通AI targetが`watchlist`となり、Portfolioへ戻る際はdefault monitoring scopeをreloadしてnamed由来のfocus / alertを残さない。
91. dashboard async responseはrequest IDだけでなくrequested `watchlist_id`と現在active scopeを照合し、切替前の遅いresponseで現在listを上書きしない。
92. named listでcheckboxを1件以上選ぶと共通AI targetが`selected`となり、全解除時にmanual tickerがなければ`watchlist`へ戻る。collectionごとの選択が混ざらない。
93. named list AIのsummary labelと別タブtitleがrequest開始時のlist名snapshotを使い、実行中 / 実行後のrenameや別list切替で変化しない。
94. selector切替、collection作成、active collection削除、active membership追加・解除、checkbox変更で既存AI結果とreader linkを無効化し、進行中の旧scope responseも表示しない。
95. list名snapshot、target自動切替、結果無効化はclient stateだけで行い、新しいAPI field、DB保存、Web Storage、追加OpenAI callを発生させない。
96. `save_result=true`の新規legacy結果だけを履歴へ追加し、`save_result=false`とcache hitでは新しい履歴recordを作らない。prompt / mock / live / raw fallbackの保存失敗時は生成回答を維持し、追加OpenAI callなしでsafe warningを1回返し、そのresponseをcacheへ保存しない。
97. legacy履歴はローカルJSONへ最大100件を保持し、不正entryを安全に読み飛ばす一方、root破損時は既存fileを上書きしない。
98. 履歴一覧APIが保存順の新しいものからmetadataだけを返し、model本文を使わない決定的summary、`mode` / `target` / `status` / `limit` / `offset`、filter後の`total`、filter前の`mode_counts`、保存総数、invalid件数を扱う。
99. 履歴詳細APIが`{history_id, review}`を返し、`request_payload`をresponseへ含めず、存在しないIDを404とする。
100. Markdown exportが安全なASCII attachment名とno-store / nosniff / no-referrer headerを持ち、可変text、source URL、raw fenceを安全に整形する。
101. dashboardが保存履歴を分析方法で分類し、metadata一覧、詳細、`.md`保存、別タブ印刷からのPDF保存を提供する。
102. 履歴一覧・詳細・export・印刷表示でOpenAIを再呼び出さず、quota / usage / cacheを更新しない。
103. 過去のcode-only履歴はread時だけ銘柄名・公開codeへ補完され、履歴fileを書き換えない。
104. 過去のnamed watchlist名は保存recordに存在しないため、現行名を推測で紐付けず、保存済みIDまたは一般的な対象labelで制約を明示する。
105. raw fallbackを画面で先頭20,000文字へ省略した場合は全文Markdown導線を示し、印刷用cloneではraw output detailsをopenにして表示範囲を欠落させない。
...
- named watchlistの利用者別owner、共有権限、招待、認証・認可
- collectionごとに異なるmemo / thesisを保持すること。memo / thesisはsecurity-level共有のままとする
- Portfolioとwatchlistの自動同期、相互変換、broker連携
- default watchlistの削除、削除済みnamed listのundo / trash / restore
- named listのexport / import、drag-and-drop並べ替え、list数のquota
- legacy保存履歴の削除、全文検索、共有URL、認証・利用者別access control
- legacy保存履歴の保持上限変更UI、期限指定、自動purge、cloud同期
- server-side PDF生成、PDF fileの保存、PDF専用download API
- 過去のlegacy保存recordに存在しないnamed watchlist名を現行collectionから推測復元すること
...
- named watchlistには利用者ID、認証、認可がなく、同じアプリDBへ接続できる利用者間でcollection名、membership、memo、thesisが共有される。loopback既定は同一PC内の利用者分離を提供しない。
- memo / thesisはcollection固有ではない。同じ銘柄を含むlistごとに別の仮説を持つ用途には対応していない。
- invalidまたは削除済み`watchlist_id`をdashboard view modelへ指定した場合はnot foundではなくdefault collectionへfallbackする。collection CRUD/item APIと検索APIのmissing IDは404であり、境界が異なる。
- non-default collection削除はundoを持たず、そのcollectionのmembership順序を復元しない。security-level rowは他membershipの有無に応じて維持・inactive化され、Portfolioは削除しない。
- explicitなnamed list AIだけが空listでmock fallbackしない。`watchlist_id`を送らないlegacy callerは従来のdefault watchlist / mock fallback互換を残す。
- default collectionの初回migrationは通常の単一application startupを前提とする。複数processが未初期化PostgreSQLへ同時起動してdefaultを競合作成する状況のserialize / retryは未実装・未検証であるため、初回DB初期化は1 processで行う。
- legacy履歴は認証のないtrusted-local機能で、同じアプリへ接続できる利用者間で回答本文を分離しない。履歴IDもaccess tokenではない。
- legacy履歴の保存上限は100件固定であり、削除・pin・保持期限・自動purge・復元を行えない。新規保存により古いrecordが上限外になる。
- history file更新の`RLock`は同一process内だけをserializeし、複数process / 複数hostの厳密な同時書き込みを保証しない。
- 保存済みnamed watchlist reviewは`watchlist_id`を持ち得るが、当時のcollection名を保存していない。rename / delete後を含む古い履歴から実行時のlist名を復元できない。
- PDFはbrowserの印刷機能に依存し、browser・OSの印刷dialog、page size、改ページ、header/footer設定をアプリから保証しない。
...
| v2.0 | 2026-08-19 | Portfolioと複数named watchlistの単一管理space、collection / membership model、default一度限りmigration、legacy default互換、scoped検索・monitoring・detail / chart・AI、checkbox target同期、list名context snapshot、stale response / 旧結果無効化、Portfolio復帰時default reload、security-level memo/thesis共有、app-global/trusted-local境界を追加 |
| v2.1 | 2026-08-19 | legacy stock-review保存履歴の最大100件、保存順の新しいものから返すmetadata一覧、mode / target / status filter、詳細envelope、request payload除外、安全なMarkdown attachment、別タブ印刷によるPDF保存、OpenAI非再呼出、cache hit非保存、破損履歴のfail-safe read、named watchlist名未保存の制約を追加 |
