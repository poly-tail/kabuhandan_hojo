...
v1.8では、v1.7の機能を累積継承し、legacy軽量スキャンで「JSON構文は正しいが項目名または型がアプリ契約と一致しない応答」を構文エラーと誤表示していた境界を修正する。legacy responseの互換aliasをcanonical fieldへ正規化し、mode別JSON SchemaとPydantic modelを一致させ、parse失敗を構文・root shape・schema validationへ分類する。生応答を表示する救済結果は分析成功とは分離し、quota成功回数とcacheを汚染しない。canonical個別銘柄AI経路とJ-Quants銘柄master契約はv1.7から変更しない。
